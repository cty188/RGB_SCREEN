...
void HUB75_Init(uint8_t width,uint8_t address_size,uint8_t bitDepth)
{
	...
	RGB_Matrix.all_width = width;
	
	RGB_Matrix.height = (2 << RGB_Matrix.address_size) * abs(RGB_Matrix.tile);

	RGB_Matrix.timer_Period = HUB75_MIN_PERIOD;
	...
	if(plane == 0){
		...
		initialBlueBit	= 0x0001 << 1 ;
			...
			for (int8_t tile = abs(RGB_Matrix.tile) - 1; tile >= 0; tile--) {
				...
				{
					lowerSrc = RGB_Matrix.BlackImage + (tile * RGB_Matrix.all_width * (RGB_Matrix.height / 2)) + RGB_Matrix.all_width * ((RGB_Matrix.height / 2) - 1 - row);
					upperSrc = lowerSrc + RGB_Matrix.all_width * (RGB_Matrix.height / 2); 
					srcIdx = RGB_Matrix.all_width - 1;                                      
					srcInc = -1;
				}
				else
				{
					upperSrc = RGB_Matrix.BlackImage + (tile * RGB_Matrix.all_width * (RGB_Matrix.height / 2)) + (RGB_Matrix.all_width * row);
					lowerSrc = upperSrc + RGB_Matrix.all_width * (RGB_Matrix.height / 2); 
					srcIdx = 0;                                     
					srcInc = 1;
				}
			
			
			for(uint16_t x = 0; x < RGB_Matrix.all_width; x++, srcIdx += srcInc)
			{
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if(htim->Instance == htim1.Instance)
    {
			...
			if(RGB_Matrix.plane == 0)
			{
				...
				if (RGB_Matrix.timer_Period < HUB75_MIN_PERIOD) RGB_Matrix.timer_Period = HUB75_MIN_PERIOD;
			}
